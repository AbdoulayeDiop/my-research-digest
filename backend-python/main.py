...
async def process_newsletter(api_client, newsletter):
    ...
    if latest_issue:
        ...
        last_issue_date = datetime.fromisoformat(
            latest_issue['publicationDate'].replace('Z', '+00:00'))
        if datetime.now(last_issue_date.tzinfo) - last_issue_date < timedelta(days=7):
            logging.info(
                f"Newsletter '{newsletter['topic']}' is up to date. Skipping.")
            return
    ...
    if result and "newsletter" in result:
        ...
        if len(papers) == 0:
            logging.warning(
                f"No papers found for topic '{newsletter['topic']}'. Skipping.")
            return
        ...
        if user_id:
            ...
            if user_info and user_info.get('email'):
                ...
                body = f"""
                    ...
                    <p>Read the issue from your dashboard: <a href="{issue_link}">{issue_link}</a></p>
                    <p>Best regards,</p>
                ...
                logging.warning(
                    f"Could not retrieve email for user {user_id} of newsletter {newsletter.get('topic')}")
        else:
            logging.warning(
                f"No userId found for newsletter {newsletter.get('topic')}")
